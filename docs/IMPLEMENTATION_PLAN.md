...
Status geral: ✅ Fases 1–13 concluídas. Pendências reais de cada fase
listadas no registro abaixo — itens que dependem de ambiente (projeto
Supabase real, chave OpenAI, Storage, provedor de e-mail) estão marcados
e entram na configuração de deploy (DEPLOYMENT.md).
...
| 10 — Nylo                    | ✅     | 2026-07-12   | (a) Funciona apenas com `OPENAI_API_KEY` configurada — sem chave, a página declara indisponibilidade honestamente (sem mock) e a API responde 503; teste ponta-a-ponta com o modelo real só no deploy; (b) 19 ferramentas (18 da tabela AI_NYLO §3 + `consultar_cotacoes` do §6, que declara ausência de provedor — `MarketDataProvider` plugável por env); (c) rascunho→confirmação implementado com endpoint normal fora do loop da IA (`origin='nylo_draft'`, auditado); rascunho v1 cobre receita/despesa/financiamento/aporte/pagamento de dívida; (d) cron de expiração de conversas (`purge_expired_ai_conversations`) pronto — agendamento na Fase 13; (e) superfícies extras (botão flutuante, painel lateral, card no Dashboard) entram na Fase 11; feedback 👍/👎 tem tabela pronta, UI fica com as superfícies; (f) tom configurável (`nylo_prefs`) fica para a tela de configurações da Nylo (Fase 11/13).                                                                                                                                                                                                      |
| 11 — Dashboard               | ✅     | 2026-07-12   | (a) Motor de alertas cobre 21 regras recomputáveis + `nylo_insight` reservada (inserida pela Nylo em versão futura); recomputação manual pelo botão do painel — agendamento diário via cron entra na Fase 13; (b) `category_limits` criada com motor e testes; a UI de definição de limites por categoria entra na página Categorias quando priorizada (hoje só via banco); (c) filtros de período do Dashboard: navegação mensal (padrão do app) — visões anual/5/10/15 anos ficam para Relatórios (Fase 12); (d) card da Nylo leva ao chat (pergunta rápida inline embarca quando houver demanda); (e) botão de privacidade borra valores via CSS e persiste no dispositivo.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 12 — Relatórios              | ✅     | 2026-07-12   | (a) Relatórios cobrem resumo do período, demonstrativo de receitas, despesas por categoria (ordenável), evolução mensal, resumo mês a mês e 50/20/30 — mesmas funções SQL do Dashboard (consistência testada: Σ meses = resumo do período); (b) CSV com BOM + `;` (Excel pt-BR) em dois formatos (categorias e mensal) via endpoint autenticado; (c) impressão via stylesheet dedicada (esconde a moldura do app); (d) resumo descritivo da Nylo pelo chat (ferramenta `gerar_relatorio` já disponível) — resumo inline na página quando a chave estiver configurada em produção; (e) relatórios por área adicional (investimentos/dívidas/patrimônio em tabela própria) podem ser adicionados sob demanda — as fontes SQL já existem.                                                                                                                                                                                                                                                                                                                                                                                       |
| 13 — Qualidade e entrega     | ✅     | 2026-07-12   | (a) Cron completo: `run_daily_maintenance` (atrasos + horizonte de recorrências + alertas de todos os workspaces + expiração de conversas) e `create_monthly_snapshots` corrigido — sob service_role gravava ZEROS; agora impersona o dono de cada workspace (bug pego por teste novo); endpoints `/api/cron/*` + `vercel.json` autenticados por `CRON_SECRET`; (b) middleware corrigido: rotas `/api` não são mais redirecionadas para /login (o redirect quebraria os crons da Vercel e devolvia HTML em vez de 401 — pego pelo E2E novo); (c) E2E ampliado de 10 para 54 execuções (proteção de TODAS as rotas privadas, APIs negando sem sessão, cron exigindo segredo, acessibilidade das páginas públicas: idioma, labels, teclado, erros anunciados); (d) fluxos E2E autenticados (17 fluxos completos) exigem projeto Supabase real — roteiro em SECURITY.md §9/PROJECT_SPEC, executar contra staging no go-live (checklist em DEPLOYMENT.md); (e) suítes finais: 43 unitários + 80 testes de banco real + 54 E2E; itens dependentes de ambiente (OpenAI, Storage, e-mail, staging) listados no checklist de deploy. |     |
